...
테스트 강의 빨리듣고 싶었는데, 떄가 아니라 남겨두다가 드디어 완강했습니다!
개인적으로 여러 프로젝트를 진행하면서 느꼈던 불편함이 내가 만든 기능에 대한 테스트를
API 라면 postman, web이라면 직접 클릭해보면서 다 테스트 해야해서 혹시나 놓친 부분들이 있지 않을까 자신감이 많이없었습니다.
테스트만으로도 어느정도 자신감을 찾은거같은데, TDD 를 직접해보니 일일이 콘솔로 찍어보지 않아도되고
중간중간에 좀 다르게 구현해도 될까? 라는 의문점이 들면 바로 시도해보고, 테스트 결과를 볼 수 있어서
시간이 촉박한 프로젝트가 아니라면 TDD는 정말 매력적이고, 오히려 개발시간을 단축시켜줄거 같다고 느꼈습니다!
